...
class tds_newsletter8 extends td_style {
    ...
    static function cssMedia( $res_ctx ) {
        ...
        // background color
        $bg_color = $res_ctx->get_style_att( 'bg_color', __CLASS__ );
        $res_ctx->load_settings_raw( 'bg_color', '#222' );
        if( $bg_color != '' ) {
            $res_ctx->load_settings_raw( 'bg_color', $bg_color );
        }
    ...
    function render( $index_style = '' ) {
        ...
        if ( ! empty($embedded_form_code) ) {
            ...
            if ( $newsletter_data === false ) {
                ...
                $buffy .= '<div class="' . self::get_group_style( __CLASS__ ) . ' ' . self::get_class_style(__CLASS__) . ' ' . $this->unique_style_class . ' ' . $input_bar_display_class . ' tdn-style-dark td-fix-index">';
