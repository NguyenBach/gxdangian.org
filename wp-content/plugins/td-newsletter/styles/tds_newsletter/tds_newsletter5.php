...
class tds_newsletter5 extends td_style {
    ...
    private function get_css() {
        ...
        $raw_css =
            "<style>
                ...
                /* @disclaimer_color */
                .$unique_style_class .tdn-disclaimer {
				...
				.$unique_style_class .tdn-icon:before {
					line-height: @icon_padding;
				}
                /* @icon_position */
                .$unique_style_class .tdn-icon-wrap {
                    margin-top: @icon_position;
                }
                /* @icon_color */
                .$unique_style_class .tdn-icon {
                    color: @icon_color;
    ...
    static function cssMedia( $res_ctx ) {
        ...
        // icon position
        $icon_position = $res_ctx->get_style_att( 'icon_position', __CLASS__);
        $res_ctx->load_settings_raw( 'icon_position', '-34px' );
        if( $icon_position != '' ) {
            if( is_numeric($icon_position) ) {
                $res_ctx->load_settings_raw( 'icon_position', $icon_position . 'px' );
            }
        }

        // icon padding
        $res_ctx->load_settings_raw( 'icon_padding', $icon_size * $res_ctx->get_style_att('icon_padding', __CLASS__) . 'px' );

        // icon color
        $icon_color = $res_ctx->get_style_att( 'icon_color', __CLASS__ );
        $res_ctx->load_settings_raw( 'icon_color', '#fff' );
        if( $icon_color != '' ) {
            $res_ctx->load_settings_raw( 'icon_color', $icon_color );
        }

        // icon background color
        $icon_bg_color = $res_ctx->get_style_att( 'icon_bg_color', __CLASS__ );
        $res_ctx->load_settings_raw( 'icon_bg_color', '#000' );
        if( $icon_bg_color != '' ) {
            $res_ctx->load_settings_raw( 'icon_bg_color', $icon_bg_color );
        }

        // icon background radius
        $icon_bg_radius = $res_ctx->get_style_att( 'icon_bg_radius', __CLASS__ );
        $res_ctx->load_settings_raw( 'icon_bg_radius', $icon_bg_radius );
        if( $icon_bg_radius != '' ) {
            if( is_numeric( $icon_bg_radius ) ) {
                $res_ctx->load_settings_raw( 'icon_bg_radius', $icon_bg_radius . 'px' );
            }
        }
        ...
        // input bar line height
        $res_ctx->load_settings_raw( 'f_input_line_height', $res_ctx->get_style_att( 'f_input_font_line_height', __CLASS__ ) );
                ...
                $buffy .= '<div class="' . self::get_group_style( __CLASS__ ) . ' ' . self::get_class_style(__CLASS__) . ' ' . $this->unique_style_class . ' ' . $input_bar_display_class . ' tdn-style-bordered td-fix-index">';
                    ...
                    $buffy .= '<div class="tdn-info-wrap">';
                        ...
                        if (!empty ($embedded_form_type) && $embedded_form_type == 'mailchimp') {
                            ...
                            $buffy .= '<form class="tdn-form" action="' . $newsletter_data['url'] . '" data-id="' . $newsletter_data['id'] . '" data-code="' . $newsletter_data['code'] . '" method="post" target="_blank">';
                        ...
                        if( $disclaimer != '' ) {
                            $buffy .= '<div class="tdn-disclaimer">' . $disclaimer . '</div>';
                        }
            ...
            */
            ...
            case 'mailerlite':
                ...
                if ( ! empty( $matched[1] ) && strpos( $matched[1], 'app.mailerlite.com/webforms') !== false ) {
                    ...
                    preg_match( '/data-id="([^"]*?)"/i', $newsletter_form_data, $matched );

                    if ( ! empty( $matched[1] ) ) {

                        $newsletter_data['id'] = $matched[1];
