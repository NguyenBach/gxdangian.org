<?php
/*
Plugin Name: Flipmag Shortcodes
Plugin URI: http://octocreation.com
Description: Flipmag Shortcodes adds multiple shortcode functionality for OctoCreation themes. 
Version: 1.0.0
Author: OctoCreation
Author URI: http://octocreation.com
Text Domain: flipmag-shortcodes
License: GPL2
*/

$flipmag_sc = Flipmag_ShortCodes::getInstance();
add_action('after_setup_theme', array($flipmag_sc, 'setup'));

/**
 * Shortcode handlers
 */
class Flipmag_ShortCodes
{
	...
	{
		// flipmag framework available? optional but good extras
		if (class_exists('Flipmag')) {
			Flipmag::options()->set('shortcodes', $this);
		}
		else {
			
			// flipmag framework themes implement it in main css for both speed and maintainability
			wp_enqueue_style('flipmag-shortcodes', plugin_dir_url(__FILE__) . 'css/shortcodes.css');
			
			// include framework dependencies
			require_once plugin_dir_path(__FILE__) . '/lib/flipmag.php';
		}
		
		// add extra shortcodes that can be enabled or disabled
		$latest_gallery = locate_template('blocks/latest-gallery.php');
				
		$this->add_blocks(apply_filters('flipmag_shortcode_default_blocks', array(
		...
		if (is_admin()) { 
			...
			$sc = new Flipmag_Admin_ShortCodes;	
		}
		...
		// i18n
		load_plugin_textdomain('flipmag-shortcodes', false, basename(dirname(__FILE__)) . '/languages');
	}
	...
	public function add_profile_fields($fields)
	{
		$fields = array_merge((array) $fields, array(
			'twitter' => __('Twitter URL', 'flipmag-shortcodes'),
			'gplus'   => __('Google+ URL', 'flipmag-shortcodes'),
			'facebook' => __('Facebook URL', 'flipmag-shortcodes'),
			'linkedin' => __('LinkedIn URL', 'flipmag-shortcodes'),
		));
	...
	{
		// add shortcodes
		$this->shortcodes = apply_filters('flipmag_shortcodes_list', $this->shortcodes); 
		...
		// preserve dropcap
		add_filter('flipmag_excerpt_pre_strip_shortcodes', array($this, 'preserve_dropcap'));
	}
	...
	{		
		...
		include apply_filters('flipmag_block_file', $block_file, $block);
     ...
     *
     */
    public function twiiterLibrary()
    {
        require_once plugin_dir_path(__FILE__) . '/lib/twitteroauth/twitteroauth.php';
    }
	 ...
	 * @todo   Switch to Flipmag_Registry for dependencies
	 */
