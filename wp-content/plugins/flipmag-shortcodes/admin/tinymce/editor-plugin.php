...
function flipmag_shortcode_editor_plugin() 
{
	 ...
	 */
	$shortcodes = Flipmag_ShortCodes::getInstance()->get_all();
	$shortcode_menu = '';
...
(function() {
	...
	?>
		...
		tinymce.PluginManager.add('flipmag_shortcodes', function(editor, url) {
				...
				var add_dialog = function(list, item, sub) {
					...
					list[list.length] = {text: item.title, onclick: function() { // onclick
						...
						// TODO: add cache
						if (item.dialog === true) {
							//tb_show('<?php _e('Shortcode: ', 'flipmag-shortcodes')?>' + item.title, '<?php echo plugin_dir_url(__FILE__) . 'shortcode-popup.php?shortcode='; ?>' + item.id);
							tb_show('<?php _e('Shortcode: ', 'flipmag-shortcodes')?>' + item.title, '<?php echo admin_url('admin-ajax.php') . '?action=flipmag_shortcode_popup&shortcode='; ?>' + item.id);
							...
							if (jQuery.trim(selected) === '') {
								selected = '<?php esc_attr_e('INSERT HERE', 'flipmag-shortcodes'); ?>'; 
							}
				...
				editor.addButton('flipmag_shortcodes', {
					type: 'menubutton',
					text: '<?php esc_attr_e('Shortcodes', 'flipmag-shortcodes'); ?>', 
					icon: false,
					menu: list
				});
		});
	...
	tinymce.create('tinymce.plugins.flipmag_shortcodes', {
		/**
		...
		init : function(ed, url) {
			...
			ed.onPostRender.add(function(ed, cm) {
				var ele = jQuery('.mceAction .mce_flipmag_shortcodesList');
				ele.html('<?php _e('Shortcodes', 'flipmag-shortcodes'); ?>');
		...
		createControl : function(n, cm) {
			...
			var add_dialog = function(list, item, sub) {
				...
				list.add({title: item.title, onclick: function() { // onclick
					...
					if (item.dialog === true) {
						...
						if (jQuery.trim(selected) === '') {
							selected = '<?php _e('INSERT HERE', 'flipmag-shortcodes'); ?>'; 
						}
			...
			if (n == 'flipmag_shortcodes') {
			
				var button = cm.createSplitButton('flipmag_shortcodesList', {
					title: '<?php echo esc_attr__('Shortcodes', 'flipmag-shortcodes'); ?>',
					icons: false
				});
	...
	// Register plugin
	tinymce.PluginManager.add('flipmag_shortcodes', tinymce.plugins.flipmag_shortcodes);
