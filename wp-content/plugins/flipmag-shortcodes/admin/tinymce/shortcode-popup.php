...
function flipmag_shortcode_popup()
{
	...
	$shortcode   = array_merge(array('label' => null, 'child' => null), Flipmag_ShortCodes::getInstance()->get_one($_GET['shortcode']));
...
.flipmag-sc-visual { margin-top: 10px; }
.flipmag-sc-visual .element-control { 
	position: relative;
	margin-bottom: 5px;
}

.flipmag-sc-visual .element-control:after {
	content: " "; 
	display: block; 
	clear: both;
}

.flipmag-sc-visual label {
	width: 150px;
	float: left;
	display: block;
	font-weight: bold;
...
.flipmag-sc-visual .element-control input {
	float: left;
	display: block;
	width: 100%;
	max-width: 200px;
...
.flipmag-sc-visual textarea { 
	width: 400px;
	height: 150px;
}

.flipmag-sc-visual .buttons {
	padding-top: 20px;
}

.flipmag-sc-visual .help {
	line-height: 2.5em;
	padding-left: 10px;
	color: gray;
	font-size: 80%;
...
<script type="text/javascript">
	...
	function Flipmag_Shortcodes_Helper($) {
		...
		this.init = function() {
			...
			this.form = $('form.flipmag-sc-visual');
		...
		this.insert_group = function(e) {
			...
			if (Flipmag_Options && $.farbtastic) {
				$('.colorpicker').wpColorPicker();
				Flipmag_Options.init_color_pickers();
			}
	...
	var Flipmag_Shortcodes_Helper = new Flipmag_Shortcodes_Helper(jQuery);

	// set shortcodes
	Flipmag_Shortcodes_Helper.shortcode = '<?php echo esc_attr(strip_tags($_GET['shortcode'])); ?>';
	Flipmag_Shortcodes_Helper.child_shortcode = '<?php echo esc_attr($shortcode['child']); ?>';

</script>

<form method="post" class="flipmag-sc-visual">
	...
	<div class="buttons">
		<input type="submit" value="<?php _e('Insert ' . $shortcode['label'], 'flipmag-shortcodes'); ?>" class="button-primary" />
	</div>
