<?php
	if(!class_exists('Vmag_Welcome')) :
		...
		class Vmag_Welcome {
			...
			public function __construct() {
				...
				$this->req_plugins = array(
					...
					'instant-demo-importer' => array(
						...
						'name' => __('Instant Demo Importer', 'vmag'),
						...
						'info' => __('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'vmag'),
					),
				...
				$this->tab_sections = array(
					'getting_started' => __('Getting Started', 'vmag'),
					'recommended_plugins' => __('Recommended Plugins', 'vmag'),
					'import_demo' => __('Import Demo', 'vmag'),
					'support' => __('Support', 'vmag'),
				);
				...
				add_action( 'load-themes.php', array( $this, 'vmag_activation_admin_notice' ) );

				/* Create a Welcome Page */
				add_action( 'admin_menu', array( $this, 'vmag_welcome_register_menu' ) );

				/* Enqueue Styles & Scripts for Welcome Page */
				add_action( 'admin_enqueue_scripts', array( $this, 'vmag_welcome_styles_and_scripts' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_vmag_plugin_installer', array( $this, 'vmag_plugin_installer_callback' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_vmag_plugin_offline_installer', array( $this, 'vmag_plugin_offline_installer_callback' ) );

				/** Plugin Activation Ajax **/
				add_action( 'wp_ajax_vmag_plugin_activation', array( $this, 'vmag_plugin_activation_callback' ) );

				/** Plugin Activation Ajax (Offline) **/
				add_action( 'wp_ajax_vmag_plugin_offline_activation', array( $this, 'vmag_plugin_offline_activation_callback' ) );
			...
			public function get_required_plugin_notification() {
				...
				$vmag_plugin_installed_notif = get_option('vmag_plugin_installed_notif');
				...
				update_option('vmag_plugin_installed_notif', $notif_counter);
				return $notif_counter;
			}

			/** Welcome Message Notification on Theme Activation **/
			public function vmag_activation_admin_notice() {
				global $pagenow;

				if( is_admin() && ('themes.php' == $pagenow) && (isset($_GET['activated'])) ) {
					?>
					<div class="notice notice-success is-dismissible">
						<p><?php printf( __( 'Welcome! Thank you for choosing %1$s! Please make sure you visit our <a href="%2$s">Welcome page</a> to get started with %1$s.', 'vmag' ), $this->theme_name, admin_url( 'themes.php?page=vmag-welcome' )  ); ?></p>
						<p><a class="button" href="<?php echo admin_url( 'themes.php?page=vmag-welcome' ) ?>"><?php _e( 'Lets Get Started', 'vmag' ); ?></a></p>
					</div>
					<?php
				}
			}
			...
			public function vmag_welcome_register_menu() {
				$action_count = get_option('vmag_plugin_installed_notif');
				$title        = $action_count > 0 ? esc_html__( 'Welcome', 'vmag' ) : esc_html__( 'Welcome', 'vmag' );
				add_theme_page( 'Welcome', $title , 'edit_theme_options', 'vmag-welcome', array( $this, 'vmag_welcome_screen' ));
			}

			/** Welcome Page **/
			public function vmag_welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
					<h1><?php printf( esc_html__( 'Welcome to %s - Version %s', 'vmag' ), $this->theme_name, $this->theme_version ); ?></h1>
					<div class="about-text"><?php printf( esc_html__( '%s is a complete Free WordPress theme for online magazines, newspapers and professional blogs. It is completely built on Customizer tool, which allows you to customize most of the theme settings easily with live previews. It is fully widgetized theme so as to let users manage the website using the easy to use widgets.', 'vmag' ), $this->theme_name ); ?></div>
				...
				<div class="nav-tab-wrapper clearfix">
					<?php foreach($tabs as $id => $label) : ?>
						...
						?>
						<a href="<?php echo admin_url('themes.php?page=vmag-welcome&section='.$id); ?>" class="<?php echo $nav_class; ?>" >
			...
			public function vmag_welcome_styles_and_scripts() {
				...
				wp_localize_script( 'accesspress-basic-welcome-screen', 'vmagWelcomeObject', array(
					'admin_nonce'	=> wp_create_nonce('vmag_plugin_installer_nonce'),
					'activate_nonce'	=> wp_create_nonce('vmag_plugin_activate_nonce'),
					'ajaxurl'		=> esc_url( admin_url( 'admin-ajax.php' ) ),
					'activate_btn' => __('Activate', 'vmag'),
					'installed_btn' => __('Activated', 'vmag'),
					'demo_installing' => __('Installing Demo', 'vmag'),
					'demo_installed' => __('Demo Installed', 'vmag'),
					'demo_confirm' => __('Are you sure to import demo content ?', 'vmag'),
				) );
			}

			/** Plugin API **/
			public function vmag_call_plugin_api( $plugin ) {
			...
			/** Check For Icon **/
			public function vmag_check_for_icon( $arr ) {
				if ( ! empty( $arr['svg'] ) ) {
			...
			public function vmag_plugin_active($plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
				$status = 'install';
			...
			public function vmag_plugin_generate_url($status, $plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
			...
			/* ========== Plugin Installation Ajax =========== */
			public function vmag_plugin_installer_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( __( 'Sorry, you are not allowed to install plugins on this site.', 'vmag' ) );
				...
				if (! wp_verify_nonce( $nonce, 'vmag_plugin_installer_nonce' ))
					wp_die( __( 'Error - unable to verify nonce, please try again.', 'vmag') );
				...
				// Get Plugin Info
				$api = $this->vmag_call_plugin_api($plugin);
			...
			public function vmag_plugin_offline_installer_callback() {
			...
			public function vmag_plugin_offline_activation_callback() {
			...
			/** Plugin Activation Ajax **/
			public function vmag_plugin_activation_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( __( 'Sorry, you are not allowed to activate plugins on this site.', 'vmag' ) );

				$nonce = $_POST["nonce"];
				$plugin = $_POST["plugin"];

				// Check our nonce, if they don't match then bounce!
				if (! wp_verify_nonce( $nonce, 'vmag_plugin_activate_nonce' ))
					die( __( 'Error - unable to verify nonce, please try again.', 'vmag' ) );
				...
				// Get Plugin Info
				$api = $this->vmag_call_plugin_api(esc_attr($plugin));
				...
				if($api->name){
					...
					$status = 'failed';
					$msg = esc_html__('There was an error activating $api->name', 'vmag');
				}
			...
			public static function get_plugin_file( $plugin_slug ) {
	      	...
	      	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=vmag-welcome&section=import_demo'),'vmag-file-installation');
	...
	if(class_exists('Instant_Demo_Importer')) :
		...
		$demoimporter->demos = array(
			'vmag' => array(
				'title' => __('Vmag Demo', 'vmag'),
				'name' => 'vmag',
				'screenshot' => get_template_directory_uri().'/welcome/demos/vmag/screen.png',
				'home_page' => 'home',
				'menus' => array(
