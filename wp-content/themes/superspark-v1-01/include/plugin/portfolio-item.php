<?php
	...
	function print_portfolio($item_xml){
		...
		if( !empty($view_all_port) && $view_all_port != 'None' ){
			...
			if( $gdl_admin_translator == 'enable' ){
				$translator_view = get_option(THEME_SHORT_NAME.'_translator_view_all_portfolio', 'View All Projects');
			}else{
				$translator_view = __('View All Projects','gdl_front_end');
			}		
	...
	function print_filter_portfolio( $port_size = "1/4", $show_title = true, $show_tag = true, $parent_category){
		...
		foreach($category_lists as $category){
			if( $category == 'All'){
				...
				echo '<li><a href="' . get_permalink() . '" class="' . $active . '">' . $translator_all . '</a><span> / </span></li>';
			}else{
				if( $current_filter == $category ) $active = 'active';
				$current_cat = get_term_by('slug', $category, 'portfolio-category');
				echo '<li><a href="' . add_query_arg(array('filter'=>$category)) . '" class="' . $active . '">'; 
	...
	function print_jquery_filter_portfolio( $port_size = "1/4", $show_title = true, $show_tag = true, $parent_category){
		...
		// start portfolio looping
		rewind_posts();
		echo '<div class="portfolio-item-holder row jquery-filter">';
		while( have_posts() ){
			the_post();		
	...
	function print_page_item($item_xml){
		...
		while( have_posts() ){
			...
			if( !empty($thumbnail_id) ){
				...
				echo '<div class="page-item-media-wrapper gdl-image">';
				echo '<a href="' . get_permalink() . '" >';
				echo '<img src="' . $thumbnail[0] .'" alt="'. $alt_text .'"/>';
				echo '</a>';
				
				echo '</div>'; //page-item-media-wrapper	
			}
			
			$show_title = (find_xml_value($item_xml, "show-title") == "Yes")? true: false;
			$show_excerpt = (find_xml_value($item_xml, "show-excerpt") == "Yes")? true: false;
			if( $show_title || $show_excerpt ){
				echo '<div class="page-item-context">';
				if( $show_title){
